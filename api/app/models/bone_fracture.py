...
Binary classification of X-ray images to detect bone fractures:
- Fractured: X-ray showing bone fracture
- Not Fractured: Normal X-ray without fracture

Covers all anatomical regions: lower limb, upper limb, lumbar, hips, knees, etc.
...
class BoneFractureClassifier(BaseClassifier):
    """Bone fracture X-ray classifier using EfficientNet-V2-S."""

    def __init__(self):
        super().__init__()
        self.model_name = "bone_fracture"
        self.image_size = 224
    ...
    def load_model(self, weights_path: str, config_path: str) -> None:
        ...
        Args:
            ...
            config_path: Path to bone_fracture_config.json
        """
        # Load config
        ...
        self.image_size = self.config.get('input_size', self.config.get('image_size', 224))
        
        # Handle both 'normalize' and 'normalization' keys
        normalize_config = self.config.get('normalize') or self.config.get('normalization', {})
        ...
        print(f"✓ Bone fracture model loaded on {self.device}")
